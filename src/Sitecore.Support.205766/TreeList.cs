...
namespace Sitecore.Shell.Applications.ContentEditor
{
  ...
  public class TreeList : Sitecore.Web.UI.HtmlControls.Control, IContentField
  {
    ...
    /// </summary>
    protected readonly TreeListFilterQueryBuilder FilterQueryBuilder;
    ...
    public virtual bool ReadOnly
    {
      get
      {
        return base.GetViewStateBool("ReadOnly");
      }
      set
      {
        base.SetViewStateBool("ReadOnly", value);
        if (value)
        {
        ...
        base.Attributes.Remove("readonly");
      }
    }
    ...
    protected void Down()
    {
      if (!this.Disabled)
      {
        ...
        {
          ListItem listItem = listbox.Controls[i] as ListItem;
        ...
        for (int j = num; j >= 0; j--)
        {
          ListItem listItem = listbox.Controls[j] as ListItem;
          Assert.IsNotNull(listItem, typeof(ListItem));
          if (listItem.Selected)
          {
            ...
            listbox.Controls.AddAt(j + 1, listItem);
          }
        }
        TreeList.SetModified();
      }
    ...
    public string GetValue()
    {
      ...
      for (int i = 0; i < items.Length; i++)
      {
        ...
        string[] array = listItem.Value.Split(new char[]
        {
                    '|'
        });
        if (array.Length > 1)
    ...
    private static bool HasItemTemplate(Item item, string templateList)
    {
      ...
      string[] array = templateList.Split(new char[]
      {
                ','
      });
    ...
    {
      return this.FilterQueryBuilder.BuildFilterQuery();
    }
    ...
    {
      return item == null || TreeList.HasItemTemplate(item, this.ExcludeTemplatesForSelection);
    }
    ...
    {
      Assert.ArgumentNotNull(item, "item");
      return this.IncludeTemplatesForSelection.Length == 0 || TreeList.HasItemTemplate(item, this.IncludeTemplatesForSelection);
    }
    ...
    private bool IsDeniedMultipleSelection(Item item, Listbox listbox)
    {
      ...
      foreach (ListItem listItem in listbox.Controls)
      {
        string[] array = listItem.Value.Split(new char[]
        {
                    '|'
    ...
    private void SetProperties()
    {
      ...
      if (@string.StartsWith("query:"))
      {
        if (Sitecore.Context.ContentDatabase != null && this.ItemID != null)
        {
          ...
          if (item != null)
          {
            ...
            try
            {
              item2 = LookupSources.GetItems(item, @string).FirstOrDefault<Item>();
            }
      ...
      else
      {
        ...
        if (this.Source != null && !@string.Trim().StartsWith("/", StringComparison.OrdinalIgnoreCase))
        {
